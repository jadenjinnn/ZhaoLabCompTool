...
class PPI(metaclass=Singleton):
    ...
    def __init__(self, update=None):
        ...
        self.__versions = {
            db.name: db.version for db in self.__sources.values()}

        # log.info(
        #     f"Integrating source databases for {self.__class__.__name__}")
        ...
        self.__proteins = pd.concat(
            [
                self.__interactome[["subject", "subjectName"]].rename(
                    columns={"subject": "proteinSymbol",
                             "subjectName": "proteinName"}
                ),
                self.__interactome[["object", "objectName"]].rename(
                    columns={"object": "proteinSymbol",
                             "objectName": "proteinName"}
                ),
            ]
