        ...
        if "http" in url:
            ...
            with open(filepath, "wb") as file:
                ftp.retrbinary(
                    ...
                    blocksize=32
                    * 1024
                    # blocksize suggested by NCBI for their database (https://ftp.ncbi.nlm.nih.gov/README.ftp)
                    * 1024,
                )
            ftp.quit()
    ...
    def retrieve_file_info(self):
        try:
            ...
            with open("data/sources/sources.json", "r+") as infofile:
                ...
                # raised by sequential_identifier (not enought files saved in sources.json for the relative database)
                except IndexError:
                    log.warning(
