...
class HPOEA:
    ...
    def __init__(
        ...
        hpoeaobj = GOEnrichmentStudy(
            hsa_gene_ids_str,  # List of human protein-coding genes
            # Gene2GoReader(
        ...
        self.__results = hpoeaobj.run_study(disease_genes_ids_str)
            ...
            pd.DataFrame(
                [
                    (
                        ...
                        -np.log10(r.p_fdr_bh),
                        r.p_bonferroni,
                        ", ".join({ncbi.get_symbol_by_id(id)
                                  for id in r.study_items}),
                    )
