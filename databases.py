...
class NCBI(Database,    metaclass=Singleton):
    ...
    def __init__(self, update=None):
        ...
        # NCBI official ids
        self.__ncbi_ids = set(
            self.__geneinfo()["GeneID"].astype({"GeneID": int}))
        ...
        self.__omim2ncbi = pd.DataFrame(
            self.__omim2ncbi, columns=["OMIM", "NCBI"])
        ...
        self.__omim2ncbi_asdict = self.__omim2ncbi.set_index("OMIM")[
            "NCBI"].to_dict()
        self.__hgnc2ncbi = pd.DataFrame(
            self.__hgnc2ncbi, columns=["HGNC", "NCBI"])
        ...
        self.__hgnc2ncbi_asdict = self.__hgnc2ncbi.set_index("HGNC")[
            "NCBI"].to_dict()
    ...
    def __init__(self, basic=False, update=None):
        ...
        def fix_go_obofile(filepath):
            ...
            filepath = os.path.join(
                os.path.dirname(
                    filepath), f"cleaned-{os.path.basename(filepath)}"
            )
        ...
        self.__gene2go = (
            self.__gene2go.merge(
                pd.DataFrame(self.__go2namespace, columns=[
                             "goTerm", "goNamespace"]),
                on="goTerm",
                how="left",
            )
        ...
        for column in ["subject", "object"]:
            self.__ontology = self.__ontology.merge(
                pd.DataFrame(self.__go2name, columns=[
            ...
            value = pd.concat(
                [
                    ...
                    self.__ontology[self.__ontology["objectType"] == namespace][
                        ...
                        columns={"object": namespace,
                                 "objectName": f"{namespace}Name"}
                    ),
                ]
            )
            setattr(
...
class DisGeNET(Database, metaclass=Singleton):
    ...
    def __init__(self, update=None):
        ...
        try:
            token = os.environ["DISGENET_API"]
        except KeyError:
            raise RuntimeError(
                "No DisGeNET API key provided"
            )
...
class LINCS(Database):  # , metaclass=Singleton
    ...
    def __init__(self, update=None, base_cell_lines=[], batch_size=None):
        ...
        self.__pertid2inchi = (
            self.__GSE70138_pert_info().set_index(
                "pert_id")["inchi_key"].to_dict()
        )
        ...
        self.__sig_perturbed_by_trt = set(
            self.__GSE70138_sig_info()[
                self.__GSE70138_sig_info()["pert_type"].isin(
                    ("trt_cp", "trt_lig"))
            ]["sig_id"].values
        ...
        self.__pertid2inchi.update(
            self.__GSE92742_pert_info().set_index(
                "pert_id")["inchi_key"].to_dict()
        )
        ...
        self.__sig_perturbed_by_trt = self.__sig_perturbed_by_trt.union(
            set(
                self.__GSE92742_sig_info()[
                    self.__GSE92742_sig_info()["pert_type"].isin(
                        ("trt_cp", "trt_lig"))
                ]["sig_id"].values
            )
            # keep only compounds, peptides and other biological agents used for treatment (https://clue.io/connectopedia/perturbagen_types_and_controls)
        ...
        def custom_read_expression_profiles(filepath):
            ...
            cols = cols[
                cols["col_name"].map(self.__sigid2cell.get).isin(
                    self.__cell_lines)
...
class HPO(Database, metaclass=Singleton):
    ...
    def __init__(self, update=None):
        ...
        for frame in tqdm(
            ...
            for clause in frame:
                if isinstance(clause, fastobo.term.NameClause):
                    self.__phenotype2name.add(
                        (f"HP:{frame.id.local}", clause.name))

        def read_gene2phenotype(filepath):
            return pd.read_csv(filepath, sep="\t", header=0, dtype=str)
        ...
        # self.__gene2phenotype = read_gene2phenotype(
        #     "./data/sources/HPO/genes_to_phenotype.txt")
...
class OMIM(Database, metaclass=Singleton):
    ...
    def search(self, query, geneMap=False):
        ...
        for start in tqdm(
            range(limit, limit *
                  (search_response["totalResults"] // limit) + 1, limit)
        ):
...
class SignaLink(Database, metaclass=Singleton):
    ...
    def __init__(self, update=None):
        ...
        self.__db = self._add_file(  # http:://signalink.org/download/521c18e9ea050e801018
            # http://signalink.org/download/33a38c88031e461c8c29 # http://signalink.org/download/e64ffdd983087ea7c794 csv # http:://signalink.org/download/33a38c88031e461c8c29 psimitab
